...
| 2026-06-15 | Partial | **HookEngine MVP landed.** `HookEngine` + `HookTypes` in `Source/CodeHarness/Hooks/`: config-driven command-type hooks, fail-open (Architecture Invariant #2), regex matcher. 11 of 13 events wired (5 in Loop, 3 in Agent, 2 in Session; Notification is glue) via `TurnInput.hookEngine` + `Agent::SetHookEngine`. `PreToolUse`/`UserPromptSubmit` are the 2 blocking events (explicit `{"action":"block"}` JSON on stdout). `[[hooks]]` parsed in ConfigManager. `SubagentStart`/`SubagentStop` deferred (no subagents yet). 9 HooksTest cases; full suite 288/288 green. `docs/customization/hooks.md` rewritten to match. **Still TODO in this plan:** PermissionManager rules DSL + `PermissionRule`/`Policy`, true Auto mode, audit logging, `callback`-type hooks, and the 2 subagent events. |
