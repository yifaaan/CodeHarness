...
class Tool {
 public:
  ...
  // 工具是否只读。
  virtual auto is_read_only() const noexcept -> bool { return false; }

  // 工具作用在哪个权限目标上。
  virtual auto permission_target(const ToolRequest&) const -> PermissionTarget { return {}; }

  virtual auto execute(const ToolRequest& request, [[maybe_unused]] const ToolContext& context) const
      -> absl::StatusOr<ToolResponse> = 0;
};
