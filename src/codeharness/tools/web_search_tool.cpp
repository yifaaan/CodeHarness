...
auto parse_web_search_input(const nlohmann::json& input) -> Result<WebSearchInput>
{
    WebSearchInput parsed;

    if (auto r = assign(parsed.query, read_json_field<std::string>(input, "query", "web_search")); !r)
    {
    ...
    {
        return fail<WebSearchInput>(
            ErrorKind::InvalidArgument,
            fmt::format("max_results must be between {} and {}", kMinResults, kMaxResults));
    }

    // search_url 可选
    auto url_result = read_optional_json_field<std::string>(input, "search_url", "web_search");
    if (!url_result)
    {
        return nonstd::make_unexpected(url_result.error());
    }
...
auto normalize_result_url(const std::string& raw_url) -> std::string
{
    ...
    if (raw_url.find("duckduckgo.com") != std::string::npos &&
        raw_url.find("/l/") != std::string::npos &&
...
auto WebSearchTool::name() const -> std::string
{
    return "web_search";
}
...
auto WebSearchTool::is_read_only() const noexcept -> bool
{
    return true;
}

auto WebSearchTool::execute(const ToolRequest& request, const ToolContext& /*context*/) const -> Result<ToolResponse>
{
