...
constexpr std::string_view kUntrustedBanner =
...
auto parse_web_fetch_input(const nlohmann::json& input) -> Result<WebFetchInput>
{
    WebFetchInput parsed;

    if (auto r = assign(parsed.url, read_json_field<std::string>(input, "url", "web_fetch")); !r)
    {
    ...
    {
        return fail<WebFetchInput>(
            ErrorKind::InvalidArgument,
            fmt::format("max_chars must be between {} and {}", kMinChars, kMaxChars));
    }

    return parsed;
}

auto validate_url(const std::string& url) -> Result<void>
{
    // 必须以 http:// 或 https:// 开头
    if (!url.starts_with("http://") && !url.starts_with("https://"))
    {
        return fail<void>(ErrorKind::InvalidArgument, "URL must start with http:// or https://");
    }
    ...
    {
        return fail<void>(ErrorKind::InvalidArgument, "URL points to internal network - SSRF blocked");
    }
...
auto WebFetchTool::name() const -> std::string
{
    return "web_fetch";
}
...
auto WebFetchTool::is_read_only() const noexcept -> bool
{
    return true;
}

auto WebFetchTool::execute(const ToolRequest& request, const ToolContext& /*context*/) const -> Result<ToolResponse>
{
    ...
    if (!validation)
    {
        return fail<ToolResponse>(validation.error().kind, validation.error().message);
    }
    ...
    if (!response)
    {
        return fail<ToolResponse>(response.error().kind, response.error().message);
    }

    auto content_type = response->headers.count("Content-Type") > 0
                            ? response->headers["Content-Type"]
                            : response->headers.count("content-type") > 0
                                  ? response->headers["content-type"]
                                  : std::string{"(unknown)"};
