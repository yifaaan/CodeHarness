...
auto parse_edit_file_input(const nlohmann::json& input) -> absl::StatusOr<EditFileInput> {
  ...
  if (parsed.old_string.empty()) {
    return absl::InvalidArgumentError("edit_file old_string must not be empty");
  }
...
auto EditFileTool::name() const -> std::string { return "edit_file"; }
...
auto EditFileTool::execute(const ToolRequest& request, const ToolContext& context) const
  ...
  if (!std::filesystem::is_regular_file(*resolved)) {
    return absl::InternalError("path is not a regular file: " + resolved->string());
  }
  ...
  if (match_count == 0) {
    return absl::InvalidArgumentError("edit_file old_string was not found");
  }
