...
//   }
auto parse_grep_input(const nlohmann::json& input) -> absl::StatusOr<GrepInput> {
  GrepInput parsed;
  ...
  if (parsed.pattern.empty()) {
    return absl::InvalidArgumentError("grep pattern is empty");
  }

  if (parsed.max_results <= 0) {
    return absl::InvalidArgumentError("grep max_results must be positive");
  }
...
auto search_file(const std::filesystem::path& workspace, const std::filesystem::path& file_path, const RE2& regex,
                 int max_results, nlohmann::json& results) -> void {
...
auto search_directory(const std::filesystem::path& workspace, const std::filesystem::path& directory, const RE2& regex,
                      int max_results, nlohmann::json& results) -> absl::Status {
...
auto GrepTool::name() const -> std::string { return "grep"; }
...
auto GrepTool::is_read_only() const noexcept -> bool { return true; }
...
auto GrepTool::execute(const ToolRequest& request, const ToolContext& context) const -> absl::StatusOr<ToolResponse> {
  ...
  if (ws_error) {
    return absl::InternalError("failed to resolve workspace path: " + ws_error.message());
  }
  ...
  if (!regex->ok()) {
    return absl::InvalidArgumentError("invalid grep pattern: " + regex->error());
  }
  ...
  if (std::filesystem::is_regular_file(search_root)) {
    ...
    return absl::InternalError("grep path is not a file or directory: " + search_root.string());
  }
