...
auto resolve_workspace_path(const std::filesystem::path& cwd, const std::filesystem::path& requested)
  ...
  if (requested.is_absolute()) {
    return absl::InvalidArgumentError("absolute paths are not allowed");
  }
  ...
  if (error) {
    return absl::InternalError("failed to resolve workspace path: " + error.message());
  }
  ...
  if (error) {
    return absl::InternalError("failed to resolve path: " + error.message());
  }

  if (!is_under_directory(base, target)) {
    return absl::InvalidArgumentError("path escapes cwd");
  }
