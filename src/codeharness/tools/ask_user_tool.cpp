...
auto AskUserTool::name() const -> std::string { return "ask_user"; }
...
auto AskUserTool::is_read_only() const noexcept -> bool { return true; }

auto AskUserTool::execute(const ToolRequest& request, const ToolContext&) const -> absl::StatusOr<ToolResponse> {
  return ToolResponse{
