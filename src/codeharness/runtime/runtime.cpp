...
auto create_provider(const ProviderConfig& config, const ToolRegistry& tools)
    -> absl::StatusOr<std::unique_ptr<ChatProvider>>;

namespace {

auto load_relevant_memories_for_prompt(const memory::MemoryStore& store, std::string_view prompt,
                                       std::size_t max_results = 5) -> absl::StatusOr<std::vector<RelevantMemory>> {
...
auto create_command_registry(SkillRegistry& skills, memory::MemoryStore& memory_store,
  ...
  return build_builtin_command_registry(skills, BuiltinCommandRegistryOptions{
...
auto create_tool_registry(const SkillRegistry& skills, coordinator::CoordinatorRuntime& coordinator_runtime)
    -> ToolRegistry {
  ToolRegistry tools;
...
class ScopedCurrentPath {
 public:
  ...
  [[nodiscard]] auto error() const -> const std::optional<absl::Status>& { return failed_; }
...
RuntimeBundle::RuntimeBundle(std::filesystem::path cwd, PermissionSettings permission, HookRegistry hooks,
                             SkillRegistryLoadResult loaded_skills, memory::MemoryStore memory_store,
                             std::unique_ptr<coordinator::CoordinatorRuntime> coordinator_runtime, ToolRegistry tools,
                             std::unique_ptr<ChatProvider> provider, std::string model, sessions::SessionStore sessions,
                             std::string provider_type, std::string base_url, std::string profile_id,
                             std::string profile_label, std::vector<RuntimeModelProfile> model_profiles)
    : cwd_(std::move(cwd)),
      ...
      commands_(create_command_registry(
          loaded_skills_.registry, memory_store_, sessions_, [this](std::string_view id) { return resume_session(id); },
      ...
      engine_(*chat_provider_, tools_, &permissions_, &hook_executor_) {
  if (model_.empty()) {
  ...
  if (model_profiles_.empty()) {
    ...
    const auto active =
        std::ranges::find_if(model_profiles_, [&](const auto& profile) { return profile.id == profile_id_; });
...
auto RuntimeBundle::cwd() const noexcept -> const std::filesystem::path& { return cwd_; }

auto RuntimeBundle::permission_mode() const noexcept -> PermissionMode { return permission_mode_; }
...
auto RuntimeBundle::current_model_profile() const -> RuntimeModelProfile {
  return RuntimeModelProfile{
      ...
      .description =
          profile_description(ProviderConfig{.type = provider_type_, .model = model_, .base_url = base_url_}),
      .provider_config =
          ProviderConfig{
              .type = provider_type_,
              .model = model_,
...
auto RuntimeBundle::plugins() const noexcept -> const std::vector<LoadedPlugin>& { return loaded_skills_.plugins; }

auto RuntimeBundle::memory_store() noexcept -> memory::MemoryStore& { return memory_store_; }

auto RuntimeBundle::memory_store() const noexcept -> const memory::MemoryStore& { return memory_store_; }

auto RuntimeBundle::commands() const noexcept -> const CommandRegistry& { return commands_; }

auto RuntimeBundle::tools() const noexcept -> const ToolRegistry& { return tools_; }

auto RuntimeBundle::coordinator_runtime() noexcept -> coordinator::CoordinatorRuntime& { return *coordinator_runtime_; }

auto RuntimeBundle::coordinator_runtime() const noexcept -> const coordinator::CoordinatorRuntime& {
  return *coordinator_runtime_;
}

auto RuntimeBundle::sessions() noexcept -> sessions::SessionStore& { return sessions_; }

auto RuntimeBundle::sessions() const noexcept -> const sessions::SessionStore& { return sessions_; }
...
auto RuntimeBundle::run_prompt(std::string_view prompt, int max_turns, const EngineEventSink& sink)
...
absl::StatusOr<std::unique_ptr<ChatProvider>> create_provider(const ProviderConfig& config, const ToolRegistry& tools) {
  ...
  if (config.type == "openai") {
    if (config.api_key.empty()) {
      return absl::FailedPreconditionError("openai provider requires an API key");
    }
  ...
  if (config.type == "anthropic") {
    if (config.api_key.empty()) {
      return absl::FailedPreconditionError("anthropic provider requires an API key");
    }
    return std::make_unique<AnthropicProvider>(config, tool_descriptions_from(tools));
  }

  return absl::InvalidArgumentError("unknown provider type: " + config.type);
}
...
auto create_runtime_bundle(RuntimeBundleOptions options) -> absl::StatusOr<std::unique_ptr<RuntimeBundle>> {
  ...
  return std::make_unique<RuntimeBundle>(
      std::move(options.cwd), std::move(options.permission), std::move(hooks), std::move(*loaded_skills),
      std::move(*memory_store), std::move(*coordinator_runtime), std::move(tools), std::move(*provider),
      options.provider_config.model, std::move(*session_store),
      options.provider_config.type.empty() ? std::string{"echo"} : options.provider_config.type,
      options.provider_config.base_url, options.active_model_profile_id, options.active_model_profile_id,
