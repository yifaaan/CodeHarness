...
public:
	...
	absl::StatusOr<StatResult> Stat(std::string_view path, bool followSymlinks = true) override;
	absl::StatusOr<std::vector<std::string>> Iterdir(std::string_view path) override;
	absl::StatusOr<std::vector<std::string>> Glob(std::string_view pattern, std::string_view path = "", const GlobOptions& options = {}) override;
