...
auto provider_retry_delay(int attempt, std::chrono::milliseconds base_delay, std::chrono::milliseconds max_delay)
...
auto provider_http_post_with_retry(std::string_view provider_name,
                                   ...
                                   int max_retries) -> absl::StatusOr<network::HttpResponse> {
  for (int attempt = 1; attempt <= max_retries; ++attempt) {
    ...
    if (!response.ok()) {
      if (attempt < max_retries) {
        ...
        spdlog::debug("{} attempt {}/{} failed (network), retrying in {}ms: {}", provider_name, attempt, max_retries,
                      delay.count(), response.status().message());
    ...
    if (should_retry_http_status(response->status_code) && attempt < max_retries) {
      auto delay = provider_retry_delay(attempt - 1);
      spdlog::debug("{} attempt {}/{} returned {} (transient), retrying in {}ms", provider_name, attempt, max_retries,
                    response->status_code, delay.count());
      std::this_thread::sleep_for(delay);
      continue;
    }
  ...
  return absl::UnavailableError(std::string{provider_name} + " exhausted retries");
}
