...
// Credentials file path: config_dir() / "credentials.json"
auto credentials_file() -> std::filesystem::path;

// Project-level config directory detection.
// Looks for .codeharness/config.toml in cwd and ancestor directories up to
...
// Deprecated: use config_file() instead. Kept for migration compatibility.
// Will be removed.
inline auto settings_file() -> std::filesystem::path { return config_file(); }
