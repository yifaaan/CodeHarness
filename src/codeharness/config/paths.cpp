...
auto skills_dir() -> std::filesystem::path
{
    return config_dir() / "skills";
}

auto plugins_dir() -> std::filesystem::path
{
    return config_dir() / "plugins";
}

auto agents_dir() -> std::filesystem::path
{
...
auto memory_dir() -> std::filesystem::path
{
    return data_dir() / "memory";
}

auto tasks_dir() -> std::filesystem::path
{
    return data_dir() / "tasks";
}

auto tool_artifacts_dir() -> std::filesystem::path
{
    return data_dir() / "tool_artifacts";
}

auto settings_file() -> std::filesystem::path
{
    return config_dir() / "settings.json";
}

auto credentials_file() -> std::filesystem::path
{
...
{
    // Walk up from cwd, looking for .codeharness/settings.json or .codeharness/ directory.
    ...
    while (true)
    {
        ...
        {
            // Found a .codeharness directory → check for settings.json or accept the dir.
            auto settings = candidate / "settings.json";
            if (std::filesystem::exists(settings, ec))
            {
                return candidate;
            }
            // .codeharness dir exists but no settings.json → still consider it a project config dir.
            return candidate;
        }
