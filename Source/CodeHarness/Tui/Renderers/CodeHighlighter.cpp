...
Color TokColor(Tok t)
{
	switch (t)
	{
		case Tok::Keyword: return Color::Magenta;
		case Tok::Type: return Color::Yellow;
		case Tok::String: return Color::Green;
		case Tok::Comment: return Color::GrayLight;
		case Tok::Number: return Color::Cyan;
		case Tok::Punct: return Color::GrayLight;
		case Tok::Plain:
...
const std::unordered_set<std::string>& KeywordTable(Family f)
{
	static const std::unordered_set<std::string> cLike = {
		...
		"await", "async", "function", "let", "var", "yield", "typeof",
		"instanceof", "in", "of", "import", "from", "as", "interface", "type",
		"extends", "implements", "package", "abstract", "synchronized",
		"defer", "go", "chan", "select", "map", "range", "func", "fallthrough",
		"crate", "fn", "impl", "match", "mut", "pub", "ref", "self", "Self",
		"trait", "where", "unsafe", "use", "move", "box", "dyn",
	};
	static const std::unordered_set<std::string> bash = {
		"if", "then", "else", "elif", "fi", "for", "do", "done", "while",
		"until", "case", "esac", "in", "function", "return", "local",
		"export", "unset", "readonly", "declare", "typeset", "shift",
		"exit", "trap", "set", "source", "echo", "printf", "read",
	};
	...
	switch (f)
	{
		case Family::CLike: return cLike;
		case Family::Bash: return bash;
		case Family::Python: return py;
		case Family::Json: return json;
		case Family::None:
		default: return none;
	}
}
...
const std::unordered_set<std::string>& TypeTable(Family f)
{
	static const std::unordered_set<std::string> cLikeTypes = {
		...
		"std::unique_ptr", "std::shared_ptr", "std::optional", "string",
		"vector", "map", "set", "pair", "variant", "optional",
	};
...
Tok ClassifyIdent(const std::string& word, Family family)
{
	...
	if (kw.find(word) != kw.end()) return Tok::Keyword;
	if (types.find(word) != types.end()) return Tok::Type;
	return Tok::Plain;
}
...
Element RenderLine(std::string_view line, Family family, const RE2& re)
{
	...
	if (spans.size() == 1) return std::move(spans[0]);
