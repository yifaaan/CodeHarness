...
{
	llm::Message msg;
	msg.role = llm::Role::Assistant;
	msg.content.push_back(llm::TextPart{"Let me check."});
	msg.toolCalls.push_back({"call_123", "read_file", "{\"path\":\"a.txt\"}"});
...
TEST_CASE("ToolsToJson: basic tool")
{
	...
	tool.inputSchema = json::parse(R"({"type":"object","properties":{"path":{"type":"string"}}})");
...
TEST_CASE("ParseStreamChunk: text content delta")
{
	...
	CHECK_FALSE(chunk->toolCallIndex.has_value());
	CHECK_FALSE(chunk->finishReason.has_value());
...
TEST_CASE("ParseStreamChunk: tool call start")
{
	...
	REQUIRE(chunk->toolCallIndex.has_value());
	CHECK(*chunk->toolCallIndex == 0);
	CHECK(chunk->toolCallId == "call_1");
	CHECK(chunk->toolCallName == "read_file");
	CHECK(chunk->toolCallArgs == "");
}
...
TEST_CASE("ParseStreamChunk: tool call argument delta")
{
	...
	REQUIRE(chunk->toolCallIndex.has_value());
	CHECK(*chunk->toolCallIndex == 0);
	CHECK_FALSE(chunk->toolCallId.has_value());
	CHECK_FALSE(chunk->toolCallName.has_value());
	CHECK(chunk->toolCallArgs == "{\"path");
}
...
TEST_CASE("ParseStreamChunk: finish reason stop")
{
	...
	REQUIRE(chunk->finishReason.has_value());
	CHECK(*chunk->finishReason == "stop");
...
TEST_CASE("ParseStreamChunk: usage in final chunk")
{
	...
	CHECK(chunk->usage->inputOther == 100);
	CHECK(chunk->usage->inputCacheRead == 0);
}
...
TEST_CASE("ParseStreamChunk: usage with cached tokens")
{
	...
	REQUIRE(chunk->usage.has_value());
	CHECK(chunk->usage->output == 50);
	CHECK(chunk->usage->inputOther == 20);
	CHECK(chunk->usage->inputCacheRead == 80);
}
...
{
	CHECK(llm::MapFinishReason("stop") == llm::FinishReason::Completed);
	CHECK(llm::MapFinishReason("tool_calls") == llm::FinishReason::ToolCalls);
	CHECK(llm::MapFinishReason("length") == llm::FinishReason::Truncated);
	CHECK(llm::MapFinishReason("content_filter") == llm::FinishReason::Filtered);
	CHECK(llm::MapFinishReason("unknown_thing") == llm::FinishReason::Other);
}
