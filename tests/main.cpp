...
public:
    ...
    auto read() -> codeharness::Result<nlohmann::json> override
    {
        if (responses_.empty())
        {
            return codeharness::fail<nlohmann::json>(
                codeharness::ErrorKind::Network, "fake MCP transport has no queued response");
        }
                ...
                nlohmann::json{
                    ...
                    {"result",
                     nlohmann::json{
                         {"content",
                          nlohmann::json::array({nlohmann::json{
                              {"type", "text"}, {"text", "echo: " + tool_arguments[tool_name].dump()}}})},
                         {"isError", false},
                     }},
                });
            return;
...
public:
    ...
    auto call_tool(std::string_view server_name, std::string_view tool_name, const nlohmann::json& arguments)
        -> codeharness::Result<codeharness::McpToolCallResult> override
    {
...
TEST_CASE("mcp tool adapter returns mcp failures as tool errors")
{
    ...
    executor.next_result =
        codeharness::fail<codeharness::McpToolCallResult>(codeharness::ErrorKind::Network, "server disconnected");
